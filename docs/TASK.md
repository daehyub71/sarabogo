...
**완료 조건:** `regions`에 시드 지역 1건이 들어가고, `lib/kto.ts`가 AppName을 부착해 관광공사 API를 1회 성공 호출한다.
...
- [ ] LLM API 키 발급 — **Q1 결정 필요** (Claude vs OpenAI)
- [ ] Supabase 프로젝트 생성
...
**완료 조건:** 로그인 사용자의 후기 작성이 지역 카드 별점·후기 수에 반영된다. 시딩 후기는 전부 `source`를 갖는다.

### 시딩 (Phase 1과 병렬 착수 — 코드보다 오래 걸림)
- [ ] 시딩 소스 수집: 지자체 공식 체험수기·결과보고서·공개 후기 (출처 URL 기록)
- [ ] 구조화 항목에 맞춰 사람이 읽고 재구성 (원문 통째 저장 금지 — 제약 C-2)
- [ ] 지역당 3~5건 · 총 50~80건 입력 (`is_seed = true`, `source` 필수)
- [ ] 시딩 후기 출처 표기 전수 점검

### 기능
- [ ] Supabase Auth 로그인 (후기 작성 시점에만 요구)
- [ ] `profiles` 생성 트리거 / `age_group` 입력
- [ ] **[Red]** 후기 저장 로직 테스트 — 별점 범위(1~5), 시딩 시 `source` 강제
- [ ] **[Green]** `ReviewForm.tsx` — 별점 5항목 + 한달실비 + 태그 + 자유서술
- [ ] 5분 내 작성 가능한지 실제 측정 (시니어 UX 검증)
- [ ] 지역별 후기 목록·집계 표시
- [ ] `regions.avg_monthly_cost` 및 평균 별점 집계 배치
- [ ] RLS 동작 검증 — 타인 후기 write 차단 테스트
