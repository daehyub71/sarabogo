...
### 선행 결정
- [x] **Q2** 시딩 대상 12개 지역 확정 (2026-07-16) — 보령·서천·강진·완도·신안·영덕·청송·남해·하동·고창·영월·정선. KTO·심평원 코드 실 API 해석 후 `seed_regions.sql`. 공공누리 발간물 전수 확인은 Q6
...
- [x] `regions` 시드 12개 지역으로 확대 (Q2 반영) — 1642개 장소 적재(전부 좌표)
...
> ⚠️ **Q6는 수작업(사람만 가능).** 검색으로는 12개 지역의 공공누리 제1유형 한달살기 발간물을
> 확정할 수 없음(2026-07-16 정찰). 각 군청 발간자료 페이지를 직접 열어 확인해야 한다.
> **소스가 얇을 수 있음** → 얇으면 인터뷰(③)·이용허락·큐레이션으로 메운다(콘솔이 4종 모두 지원).
>
> **지역별 확인 경로 (Q6 체크리스트):**
> 1. `{군}청 → 정보공개/열린데이터/발간자료`에서 "한달살기" "체험수기" "결과보고서" 검색
> 2. 문서 하단 **공공누리 마크 유형 확인** — 제1유형만 채택 (제2·4유형 폐기)
> 3. "제3자 저작권 포함 부분 제외" 문구가 **참가자 수기 본문에 걸리는지** 건별 확인
> 4. 통과분만 관리자 콘솔 `origin='public_doc'`으로 입력 (기관명·연도·저작물명·URL 기록)
- [ ] 12개 지역 발간자료 페이지 전수 확인 (강진·보령·남해 등 한달살기 유명 지역 우선)
...
- [x] `profiles.role` + `lib/authz` 관리자 검사 (FR-5.1). Supabase Auth(@supabase/ssr) 로그인
- [x] 비관리자·미로그인 `/admin` → `/login` 리다이렉트 (헤드리스 검증: 307)
- [x] `admin/reviews/new` — **origin 선택 → 분기 폼** (FR-5.2)
  - [x] `public_doc`: 출처 4종 + 제1유형 고정(`KOGL-1`) + 제3자 저작권 확인 체크
  - [x] `licensed`: 원문 URL + 동의서 URL 필수
  - [x] `curated`: **원문 붙여넣기 필드 없음**(헤드리스 확인) · 사실 항목 + `source_url`/`source_domain` · 총평 400자
  - [x] `interview`: 대상자 동의 확인
- [x] origin별 DB CHECK 위반 시 저장 실패 (SQL 직접 검증: 401자·제2유형·동의서누락 전부 거부)
- [x] `admin_audit` 기록 — 모든 쓰기에 행위자·근거 스냅샷 (FR-5.6, 실저장 확인)
- [ ] `admin/reviews/verify` — 검수 큐, 승인 시 `verified_by`/`verified_at` 기록 (FR-5.5) — 현재 입력=검수 단일단계
